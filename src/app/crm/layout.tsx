...
// Herramienta interna (no publica): solo ADMIN y EQUIPO entran aqui. No
// comparte navegacion con el marketplace (Navbar/Footer) a proposito, para
// que se sienta como una app aparte, como pediste ("con su propia URL").
export default async function CrmLayout({ children }: { children: React.ReactNode }) {
  const usuario = await obtenerUsuarioActual();
  if (!esEquipoCrm(usuario)) redirect("/login");
        ...
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Link href="/crm" className="flex items-center gap-2">
              <Logo size={22} />
              <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">CRM</span>
            </Link>

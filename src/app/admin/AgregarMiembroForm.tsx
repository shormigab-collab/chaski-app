...
// Formulario para que un ADMIN cree una cuenta de equipo (acceso al CRM
// interno en /crm). No pasa por /registro porque esas cuentas no son
// clientes ni proveedores del marketplace.
export default function AgregarMiembroForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setCargando(false);
    if (res.ok) {
      setExito(`Cuenta creada para ${email}. Ya puede iniciar sesión y entrar a /crm.`);
      setNombre("");
      setEmail("");
      setPassword("");
      router.refresh();
  ...
  return (
    <form onSubmit={onSubmit} className="border rounded-xl p-5 grid sm:grid-cols-4 gap-3 items-end">
      ...
      <div className="sm:col-span-1">
        ...
        >
          {cargando ? "Creando..." : "Agregar al CRM"}
        </button>
      </div>
